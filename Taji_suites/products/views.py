    ...
    if request.method == "POST":
        with transaction.atomic():
            ...
            for key, value in request.POST.items():
                ...
                print(f"Received order for dept: {dept}, obj_id: {obj_id}, qty: {qty}")  # Debug

                products_to_add = []

                # 🔹 Bar user can order both bar and kitchen
                if user.department == "bar":
                    bar_product = BarStockSheet.objects.filter(id=obj_id, date=today).first()
                    kitchen_product = KitchenStockSheet.objects.filter(id=obj_id, date=today).first()

                    if bar_product:
                        products_to_add.append(bar_product)
                    if kitchen_product:
                        products_to_add.append(kitchen_product)

                # 🔹 LNK user only orders LNK products
                elif user.department == "lnk":
                    lnk_product = LNKStockSheet.objects.filter(id=obj_id, date=today).first()
                    if lnk_product:
                        products_to_add.append(lnk_product)
                ...
                for product in products_to_add:
                    print(f"Processing {product.item} with qty {qty}")  # Debug
...
def take_stock(request):
    ...
    if request.method == 'POST':
        ...
        # âœ… Handle new product creation
...
def edit_bar_stock(request, stock_id):
    ...
    if request.method == "POST":
        ...
        stock.save()
        return redirect('bar_stock_page')  # ðŸ‘ˆ change to your URL name
...
def generate_today_bar_stock(request):
    if request.method == "POST":
        ...
        # ðŸ”¹ Get latest stock per item (yesterday or last available)
        latest_stocks = []
        ...
        messages.success(request, f"âœ… {created_count} stock records generated for today!")
        return redirect('bar_stock_page')
    else:
...
def edit_lnk_stock(request, stock_id):
    ...
    if request.method == "POST":
        ...
        stock.save()
        return redirect('lnk_stock_page')  # ðŸ‘ˆ change to your URL name
...
def edit_kitchen_stock(request, stock_id):
    ...
    if request.method == "POST":
        ...
        stock.save()
        return redirect('kitchen_stock_page')  # ðŸ‘ˆ change to your URL name
